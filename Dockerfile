FROM ingensi/oracle-jdk MAINTAINER Ingensi labs <contact@ingensi.com> RUN yum update -y && yum install -y unzip
...
ENV PATH $PATH:/activator-1.3.6 EXPOSE 9000 8888 RUN mkdir /app
WORKDIR /app
USER 1000
CMD ["activator", "run"]
